<x-filament-panels::page>
    ...
    <x-filament::section>
        <x-slot name="heading">Payables aging as of {{ $asOf }}</x-slot>
        <x-slot name="description">Open supplier invoices with payments applied (oldest first). Reconciles with the AP control account.</x-slot>
                    ...
                    <tbody>
                        @foreach ($aging['rows'] as $row)
                            <tr class="border-b">
                                ...
                                <td class="py-2 pl-3">
                                    <a href="{{ route('print.supplier-statement', $row['party_id']) }}" target="_blank"
